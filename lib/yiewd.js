...
Driver.prototype._setInnerDriver = function(driver) {
  this.driver = driver;
  var allMethods = this.getDriverMethods()
    , driverMethods = allMethods[0]
    , singleElMethods = allMethods[1];

  _.each(driverMethods, function(asyncMethod) {
    this[asyncMethod] = this.wrapAsyncMethod(this.driver, asyncMethod);
  }.bind(this));

  _.each(singleElMethods, function(asyncMethod) {
    this[asyncMethod] = this.wrapSingleElMethod(this.driver, asyncMethod);
  }.bind(this));
...
Driver.prototype.wrapSingleElMethod = function(wrappedObj, asyncMethod) {
  var self = this;
  var singleElFn = function() {
    var args = self.unwrapAnyElements(apsc(arguments));
    var monocleCb = o_C();
    var actionCb = null;
    var cb = function() {
      var singleElRes = apsc(arguments);
      var err = singleElRes[0];
      var el = singleElRes[1];
      if (actionCb !== null) {
        actionCb(err, el);
        actionCb = null;
      } else {
        monocleCb(err, self.wrapAnyElements(el));
      }
    };
    _.each(Element._getElementMethods(), function(elMethod) {
      monocleCb[elMethod] = function() {
        var monocleActionCb = o_C();
        var actionArgs = apsc(arguments);
        actionCb = function(err, el) {
          if (err) return monocleActionCb(err);
          actionArgs.push(function() {
            var actionRes = apsc(arguments);
            monocleActionCb(actionRes[0], self.wrapAnyElements(actionRes[1]));
          });
          el[elMethod].apply(el, actionArgs);
        };
        return monocleActionCb;
      };
    });
    args.push(cb);
    wrappedObj[asyncMethod].apply(wrappedObj, args);
...
Driver.prototype.getDriverMethods = function() {
  ...
  _.each(wdUtils.elementFuncTypes, function(type) {
    singleElMethods.push("element" + wdUtils.elFuncSuffix(type));
  });
  _.each(wdUtils.elementFuncTypes, function(type) {
  ...
  return [methods, singleElMethods];
};
